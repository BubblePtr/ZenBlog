...
- 抖动触发区在标注本身而非词上：伪元素开 `pointer-events: auto` 后，悬停/点击箭头和标签都算悬停链接；词文本包一层 `.ann-word` span，抖动选择器用 `:not(:has(.ann-word:hover))` 排除来自词的悬停，于是悬停词只染色、悬停标注才抖动。键盘 focus 词时照常抖动（键盘够不到标注，focus 是它的等价物）。
- 抖动本身：词不动，箭头与标签从慢摇渐入快颤（慢摇 400ms×2 → 加快 200ms×3 → 快颤 110ms 循环）。抖动动画追加在入场动画同名列表之后，入场不会因悬停开始/结束而重播；位移用独立的 rotate/translate 属性，与库的定位 transform 叠加。`prefers-reduced-motion` 下关闭。
