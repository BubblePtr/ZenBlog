...
首页不靠顶栏导航。身份区下面是一段介绍站点有什么的正文，正文里「项目」「文章」「摄影」「关于」这几个词就是入口，纸边的手写旁注和箭头把它们标出来。旁注在这里从「讲解」变成了「路由」。

```astro
<AnnotatedNav>
  I ship <Note href="/projects/" note="4 shipped, 1 building">small products</Note>, …
</AnnotatedNav>
```

- **词是链接，注是批注。** 正文里的词用青竹下划线，是真正的 wayfinding；旁注是指向它的铅笔批注，写词本身没说的信息（数量、语言、状态），不重复词义。旁注整块可点，作为扩大的命中区域，但颜色仍是墨次级，不用青竹。
- 一段最多四处注（项目、文章、摄影、关于），每处不超过八个词。首页签名旁注并入这一组，指向「关于」这个词，于是全站不再有无所指的旁注。
- 箭头是手绘感的 SVG 曲线（1.5px 描边、圆头），从注文指向词，位置由脚本按词的实际坐标计算，字体加载后与窗口变化时重算。
- 首页顶栏保留语言切换与主题切换，隐藏四个栏目链接；其它页面顶栏不变。
- 720px 以下不画旁注和箭头，只保留正文里的链接词，顶栏栏目链接恢复显示。
- 旁注的具体落位（右栏、行间浮注、行内圈注）由原型对比决定，记录在 `docs/design/decisions/annotated-nav.md`。

### 5.3 签名旁注（并入 5.2）

`indie since Jun 2026` 是标注导航里指向「关于」的那一处注，不再单独存在。全站每一处旁注都必须指向具体内容。
