...
本仓库使用标准 Git 管理（jj 已移除，仓库恢复为纯 Git 仓库）。

### 约定

- 提交信息遵循 Conventional Commits。
- 不直接推送到 `main`；一律通过 feature 分支（`feat/...`、`fix/...`、`chore/...`）+ PR 合并。
- 开新工作前：`git checkout main && git pull`，然后 `git checkout -b feat/xxx`。
- PR 的创建/合并用 `gh` CLI。
- PR 合并后：切回 `main`、`git pull`，删除已合并的本地分支（可选删远端分支）。
