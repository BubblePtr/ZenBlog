...
<Callout type="info" title="Did you know?">
  Astro's MDX integration supports GitHub Flavored Markdown (GFM) by default, including tables and
  task lists.
</Callout>

<Callout type="warning" title="Performance Tip">
  While MDX is powerful, note that every embedded React component adds to the page's JS bundle size.
  Use `client:visible` accordingly.
</Callout>

<Callout type="error" title="Common Mistake">
  Don't define complex React component logic directly in MDX. It's better to extract them into
  separate `.tsx` files and import them to keep your articles clean.
</Callout>
